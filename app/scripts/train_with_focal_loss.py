...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # inputs: (バッチサイズ, クラス数), targets: (バッチサイズ)
...
def evaluate(model, test_loader, device, figure_savename):
    ...
    cm = confusion_matrix(all_labels, all_preds); plt.figure(figsize=(8, 6)); sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=class_names, yticklabels=class_names); plt.title('Confusion Matrix (Focal Loss)'); plt.ylabel('Actual'); plt.xlabel('Predicted')
...
def run_focal_loss_training():
    ...
    for epoch in range(PRETRAIN_EPOCHS):
        ...
        for inputs, labels in progress_bar:
            inputs, labels = inputs.to(device), labels.to(device); optimizer.zero_grad(); outputs = model(inputs); loss = criterion(outputs, labels); loss.backward(); optimizer.step(); progress_bar.set_postfix(loss=f"{loss.item():.4f}")
    
    print("Pre-training finished.")

    # --- 2. 凍結ファインチューニングフェーズ ---
    finetune_loader = prepare_finetune_loader()
    for param in model.conv_layers.parameters(): param.requires_grad = False
    ...
    for epoch in range(FINETUNE_EPOCHS):
        ...
        for inputs, labels in progress_bar:
            inputs, labels = inputs.to(device), labels.to(device); optimizer_ft.zero_grad(); outputs = model(inputs); loss = criterion(outputs, labels); loss.backward(); optimizer_ft.step(); progress_bar.set_postfix(loss=f"{loss.item():.4f}")
    ...
    for epoch in range(UNFROZEN_FINETUNE_EPOCHS):
        ...
        for inputs, labels in progress_bar:
            inputs, labels = inputs.to(device), labels.to(device); optimizer_full.zero_grad(); outputs = model(inputs); loss = criterion(outputs, labels); loss.backward(); optimizer_full.step(); progress_bar.set_postfix(loss=f"{loss.item():.4f}")

    print("All training phases finished.")

    # --- 最終評価 ---
    X_test = np.load(os.path.join(PROCESSED_DATA_DIR, 'X_test.npy')); y_test = np.load(os.path.join(PROCESSED_DATA_DIR, 'y_test.npy'))
    test_dataset = ECGDataset(X_test, y_test, transform=None); test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=0)
    
    model_savename = 'best_model_focal_loss.pth'
    figure_savename = 'confusion_matrix_focal_loss.png'
